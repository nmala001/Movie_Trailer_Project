...
def _convert_to_lower(dictionary):
    ...
    # We use it in the omdb_search to convert the in the API response dictionary to lowercase before the function returns
    ...
    """
      ...
      -We use Google API client library in python and a YOUTUBE API key
       to make API calls to search for the movie trailers.
        ...
        -We use the build service object to make API calls specific to movie trailers(see the 'payload' variable below)

    Args:
        title(str): The title of the movie to search for
        *args(str or None): More words to add to query like the Movie release year

    Returns:
        str: The youtube video ID that corresponds to top search result.
...
def omdb_movie_info(title, year=None):
    ...
    """
      ...
      - We use this function to create a request payload if a valid OMDB API key is given using the request library.
